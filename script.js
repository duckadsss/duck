...
function startBattlePolling() {
    ...
    arenaState.battlePollingInterval = setInterval(async () => {
        ...
        if (res.status === 'waiting') {
            const searchStatus = document.getElementById('arenaSearchStatus');
            if (searchStatus) {
                searchStatus.innerHTML = '⏳ Ожидание соперника...';
            }
            // ✅ FIX #2: Пока ждём — сбрасываем confirmationShown,
            // чтобы модалка точно показалась когда статус станет active
            arenaState.confirmationShown = false;
        }
        
        if (res.status === 'active' || res.status === 'pending_confirmation') {
            // ✅ FIX #3: needConfirmation — проверяем именно СВОЁ подтверждение,
            // независимо от того, подтвердил ли соперник
            const myConfirmed = res.isPlayer1 ? res.player1Confirmed : res.player2Confirmed;
            const needConfirmation = !myConfirmed;
            
            if (needConfirmation && !arenaState.confirmationShown) {
                console.log('🔔 Показываем модалку подтверждения, isPlayer1:', res.isPlayer1);
                arenaState.confirmationShown = true;
                showNativeBattleConfirmation(res);
            } else if (!needConfirmation && arenaState.confirmationShown) {
                // Я уже подтвердил — жду соперника, сбрасываем флаг
                arenaState.confirmationShown = false;
            }
