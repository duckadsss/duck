// ============================================================
// DNA MMO - ПОЛНАЯ КЛИЕНТСКАЯ ЧАСТЬ (С АРЕНОЙ И ВЕБХУКАМИ)
// ============================================================
...
let arenaState = {
    selectedTeam: [],
    currentBattle: null,
    battlePollingInterval: null, // оставляем для совместимости, больше не используется
    isSearching: false,
    battleMoveTimeout: null,
    confirmationShown: false,
    battleActive: false,
    sseConnection: null   // EventSource соединение
};
...
async function findMatch() {
    ...
    if (res?.success) {
        ...
        // ✅ FIX #1: Polling запускается ВСЕГДА при успешном find-match,
        // и для waiting (player1), и для active (player2).
        // Раньше player1 никогда не запускал polling!
...
function initArenaWebhooks() {
    ...
    // Устанавливаем обработчик для кнопок в попапах
...
// Активация главной кнопки в зависимости от вкладки
...
// Показ нативного попапа для подтверждения боя
function showNativeBattleConfirmation(battleData) {
    ...
    tg.showPopup({
        ...
        message: `Противник: ${battleData.opponent?.name || 'Игрок'} (УР ${battleData.opponent?.level || '?'})\n` +
                 ...
                 `Принять бой?`,
        buttons: [
            { id: 'accept', type: 'default', text: '✅ ПРИНЯТЬ', color: '#22c55e' },
            { id: 'reject', type: 'destructive', text: '❌ ОТКЛОНИТЬ' }
        ]
    });
}

// Fallback HTML модалка
function showBattleConfirmationHTML(battleData) {
    ...
    popup.innerHTML = `
        ...
        <div style="background:#0d1120;border-radius:12px;padding:12px;margin:12px 0;">
            ...
            <div style="font-size:20px;font-weight:700;color:var(--legendary)">${battleData.prizePool} MMO</div>
        </div>
        <div class="modal-buttons" style="display:flex;gap:10px;">
            <button class="popup-btn" style="flex:1;background:linear-gradient(135deg,#22c55e,#16a34a)" onclick="acceptBattleWebhook()">
                <i class="fa-solid fa-check"></i> Принять
            </button>
            <button class="popup-btn" style="flex:1;background:#1a2540;color:#e2e8f0" onclick="rejectBattleWebhook()">
...
function showNativeBattleResult(isWin, prizePool) {
    if (!window.Telegram?.WebApp) {
        ...
        popup.innerHTML = `
            ...
            <div class="popup-subtitle">
                ${isWin ? `Вы выиграли ${prizePool} MMO!` : 'В следующий раз повезёт!'}
            </div>
    ...
    const message = isWin 
        ? `Вы выиграли ${prizePool} MMO! 🎉\nВаш рейтинг повышается!`
...
async function rejectBattleWebhook() {
    ...
    if (battleRes?.battleId) {
        ...
        if (window.Telegram?.WebApp) {
            window.Telegram.WebApp.showToast({ text: 'Бой отклонён' });
        } else {
...
async function renderArenaFightTab() {
    ...
    if (battleRes?.hasBattle && battleRes.status !== 'finished') {
        ...
        else if (battleRes.status === 'active' || battleRes.status === 'pending_confirmation') {
            const myConfirmed = battleRes.isPlayer1 ? battleRes.player1Confirmed : battleRes.player2Confirmed;
            const needConfirmation = !myConfirmed;
...
async function cancelBattleSearch() {
    const battleRes = await apiRequest('GET', '/api/arena/battle/status');
    if (battleRes?.battleId && battleRes.status === 'waiting') {
        await apiRequest('POST', '/api/arena/surrender', { battleId: battleRes.battleId });
    }
    stopArenaSSE();
    arenaState.isSearching = false;
    arenaState.confirmationShown = false;
...
// Функция переключения вкладок арены
...
function startArenaSSE() {
    stopArenaSSE(); // закрываем старое если есть
    
    if (!state.token) return;
    
    console.log('📡 Подключаем SSE...');
    
    // EventSource не поддерживает заголовки напрямую —
    // передаём токен через query параметр
    const url = `/api/arena/events?token=${encodeURIComponent(state.token)}`;
    ...
    // Матч найден — показываем модалку подтверждения
    ...
    // Один из игроков подтвердил — обновляем статус
    sse.addEventListener('confirmation_update', (e) => {
        ...
        if (myConfirmed) {
            // Я уже подтвердил, жду соперника
            arenaState.confirmationShown = false;
            showToast('Ожидаем соперника...', '⏳');
        }
    });
    
    // Оба подтвердили — запускаем бой
    sse.addEventListener('battle_start', (e) => {
        ...
        // Формируем данные для renderBattleInterface
    ...
    // Соперник сделал ход — обновляем интерфейс
    sse.addEventListener('move_update', (e) => {
        const data = JSON.parse(e.data);
        console.log('🗡️ SSE move_update:', data);
        if (!arenaState.battleActive) return;
        
        // Формируем данные совместимые с updateBattleInterface
        const isPlayer1 = data.player1Team && arenaState.currentBattleIsPlayer1;
    ...
    sse.onerror = (e) => {
        ...
        stopArenaSSE();
        // Переподключаемся только если поиск или бой активны
...
// Оставляем startBattlePolling как no-op для обратной совместимости
function startBattlePolling() {
    startArenaSSE();
}

// ✅ Обновление боя без полной перерисовки DOM (нет мигания)
function updateBattleInterface(battleData) {rArenaFightTab();
        }
    }, 1000);
}
...
function renderBattleInterface(battleData) {
    ...
    arenaState.battleActive = true;
    arenaState.currentBattleIsPlayer1 = battleData.isPlayer1; // для SSE move_update
...
// Анимация входящего урона (когда соперник бьёт нас) — для SSE move_update
function showIncomingDamageAnimation(lastMove) {
    if (!lastMove) return;
    // Находим нашего первого живого существа (упрощённо targetIndex для своей команды)
    const myCreatures = document.querySelectorAll('#arenaMyCreatures .arena-battle-creature');
    if (!myCreatures.length) return;
    const target = myCreatures[lastMove.targetIndex] || myCreatures[0];
    if (!target) return;
    const damageDiv = document.createElement('div');
    damageDiv.className = `arena-damage-number ${lastMove.isCrit ? 'crit' : ''}`;
    damageDiv.textContent = `-${lastMove.damage}`;
    target.style.position = 'relative';
    target.appendChild(damageDiv);
    setTimeout(() => damageDiv.remove(), 1000);
    if (lastMove.isCrit) {
        target.style.animation = 'damageShake 0.3s ease';
...
window.findMatch = findMatch;                              // ← ДОБАВИТЬ ЭТУ СТРОКУ
