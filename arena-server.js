// ============================================================
// arena-server.js - Серверная логика PvP арены
// ============================================================
...
class ArenaBattleManager {
    constructor(battleModel, userModel, arenaStatsModel, getCreatureFn, sendNotificationFn) {
    ...
    async findMatch(user, teamIds) {
        ...
        if (waitingBattle) {
            ...
            // Важно: отмечаем изменения в массивах
    ...
    async processMove(battleId, userId) {
        ...
        // Важно: отмечаем изменения в командах
        if (isPlayer1) {
    ...
    async expireOldBattles() {
        ...
        for (const battle of expiredWaiting) {
            ...
            // Возвращаем entry fee первому игроку для waiting боя
            ...
            // Очищаем currentBattleId для обоих игроков
            if (player1Id) {
...
class ArenaSSEManager {
    ...
    add(userId, res) {
        ...
        const existing = this.clients.get(userIdStr);
        if (existing) {
            try { existing.end(); } catch (_) {}
        }
        this.clients.set(userIdStr, res);
        console.log(`📡 SSE подключён: ${userIdStr} (всего: ${this.clients.size})`);
    }
    
    remove(userId) {
        this.clients.delete(userId.toString());
        console.log(`📡 SSE отключён: ${userId} (всего: ${this.clients.size})`);
    }
    
    send(userId, event, data) {
        const res = this.clients.get(userId.toString());
        if (!res) return false;
