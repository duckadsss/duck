...
function startBattle(battleData) {
    ...
    document.getElementById('arenaMainUI').style.display = 'none';
    document.getElementById('arenaBattleScreen').style.display = 'block';
    
    // Set player names
    document.getElementById('battlePlayerName').innerHTML = `YOU <span style="font-size:10px">(${battleData.player.rating})</span>`;
    document.getElementById('battleOpponentName').innerHTML = `${battleData.opponent.name} <span style="font-size:10px">(${battleData.opponent.rating})</span>`;
...
function arenaAttack(attackerIndex) {
    ...
    // Show target selection UI briefly
...
function showTargetSelection(attackerIndex) {
    ...
    opponentMonsters.forEach(card => {
        const monster = arenaCurrentBattle.opponent.monsters[parseInt(card.dataset.index)];
        if (monster && monster.hp > 0) {
            card.classList.add('targetable');
            card.style.cursor = 'pointer';
            card.onclick = () => {
                const targetIndex = parseInt(card.dataset.index);
                confirmAttack(attackerIndex, targetIndex);
...
function clearTargetSelection() {
    ...
    if (window.tempTargetCallback) window.tempTargetCallback();
}
...
function confirmAttack(attackerIndex, targetIndex) {
    ...
    if (!attackerMonster || attackerMonster.hp <= 0) {
        addBattleLogMessage('❌ Your monster is defeated!', true);
        return;
    }
    
    if (!targetMonster || targetMonster.hp <= 0) {
        addBattleLogMessage('❌ Target is already defeated!', true);
        return;
    }
...
function updateBattleMonsters(side, monsters) {
    monsters.forEach((monster, idx) => {
        ...
        if (card) {
            ...
            if (hpText) hpText.innerHTML = `${monster.hp}/${monster.maxHp}`;
...
function closeBattle() {
    ...
    document.getElementById('arenaBattleScreen').style.display = 'none';
    document.getElementById('arenaMainUI').style.display = 'block';
