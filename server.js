...
// server.js - ДОБАВЛЯЕМ НОВЫЕ ПОЛЯ

const ArenaStatsSchema = new mongoose.Schema({
    userId: { type: mongoose.Schema.Types.ObjectId, ref: 'User', required: true, unique: true },
    rating: { type: Number, default: 1000 },
    league: { type: String, default: 'bronze' },
    peakRating: { type: Number, default: 1000 },        // Максимальный рейтинг за всё время
    promotions: { type: Number, default: 0 },           // Количество повышений в лиге
    demotions: { type: Number, default: 0 },            // Количество понижений
    promotionProtection: { type: Boolean, default: true }, // Защита от падения после повышения
...
setInterval(() => {
    ...
    // FIX: очищаем истёкшие admin-сессии (иначе Map растёт бесконечно)
...
// HEALTH CHECK
// ============================================
app.use(express.static(__dirname));app.get('/health', (req, res) => {
    ...
    try {
        ...
        if (!user) {
            ...
            const notificationMessage = `🆕 <b>НОВЫЙ ИГРОК!</b>\n\n` +
                ...
                `📛 Имя: ${escapeHtmlTg(userData.first_name || '?')} ${escapeHtmlTg(userData.last_name || '')}\n` +
                `🔗 Username: ${userData.username ? '@' + escapeHtmlTg(userData.username) : 'нет'}\n` +
...
app.post('/api/arena/team', authMiddleware, async (req, res) => {
    try {
        ...
        for (const creatureId of team) {
            if (!inventoryMap.has(creatureId) || inventoryMap.get(creatureId) < 1) {
...
app.post('/api/arena/find-match', authMiddleware, async (req, res) => {
    try {
        if (!arenaManager) return res.status(503).json({ success: false, message: 'Арена инициализируется, попробуйте позже' });
        const user = req.user;
        ...
        if (user.currentBattleId) {
            ...
            if (existingBattle && ['waiting', 'active'].includes(existingBattle.status)) {
...
app.get('/api/arena/battle/status', authMiddleware, async (req, res) => {
    ...
    try {
        if (!arenaManager) return res.status(503).json({ success: false, hasBattle: false, message: 'Арена инициализируется' });
        const status = await arenaManager.getBattleStatus(req.user._id);
    ...
    try {
        ...
        if (result.bothConfirmed) {
            ...
            arenaSocketManager.send(battle.player1Id, 'battle_start', {
                ...
                lastMoveAt: battle.lastMoveAt  // FIX: для синхронизации таймера
            });
    ...
    try {
        ...
        res.json({ 
            ...
            leaders: leaders.map(l => ({
                ...l,
                league: l.league || 'bronze'  // Добавляем лигу каждому игроку
            })),
            myRank, 
            myStats: {
...
app.get('/api/arena/history', authMiddleware, async (req, res) => {
    try {
        ...
        const history = battles.map(battle => {
            // FIX: player1Id/_id может быть null если юзер удалён
            const p1id = battle.player1Id?._id?.toString() || battle.player1Id?.toString();
...
// ============================================
// АРЕНА - ДОПОЛНИТЕЛЬНЫЙ ЭНДПОИНТ
// ============================================
app.post('/api/arena/check-battle-status', authMiddleware, async (req, res) => {
    try {
        const { battleId } = req.body;
        const battle = await ArenaBattle.findById(battleId);
        if (!battle) {
            return res.json({ success: false, message: 'Бой не найден' });
        }
        
        const isPlayer1 = battle.player1Id.toString() === req.user._id.toString();
        res.json({
            success: true,
...
app.post('/api/wallet/get-payment-details', authMiddleware, async (req, res) => {
    try {
        ...
        const TON_WALLET = process.env.TON_WALLET || 'UQAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA';
...
// ============================================
// АДМИН-ПАНЕЛЬ API (сокращённо, основные эндпоинты)
// ============================================
...
io.use(async (socket, next) => {
    try {
        ...
        console.log(`🔐 WebSocket аутентификация, token: ${token ? 'present' : 'missing'}`);
        ...
        socket.user = user;
        console.log(`✅ WebSocket аутентификация успешна: ${user.telegramId}`);
        next();
    } catch (err) {
...
// ЗАПУСК
// ============================================
// 1. В mongoose.connection.once('open') - изменить порядок инициализации:
mongoose.connection.once('open', async () => {
    await initCreatures();
    await getGameConfig();
    
    // Сначала создаем socketManager (НО io уже должен быть создан)
    // Проблема: io создается ПОСЛЕ этого блока? Нет, io создается до.
    // Но arenaSocketManager использует io, который уже существует.
