// ============================================
// server.js - ПОЛНАЯ ВЕРСИЯ С PvP АРЕНОЙ + РАССЫЛКИ + РАСШИРЕННАЯ АДМИНКА
// ============================================
...
app.get('/', (req, res) => {
    res.json({ success: true, message: '🚀 DNA MMO Backend работает!', version: '6.0.0' });
});
    ...
    try {
        ...
        const updatedUser = await User.findOneAndUpdate(
            { _id: user._id, balance: { $gte: amount } },
            {
                ...
                $push: {
                    transactions: {
                        $each: [{ name: `Withdraw request: ${amount} MMO`, amount: -amount, time: new Date() }],
                        $position: 0,
                        $slice: 30
                    }
                }
        ...
        res.json({ success: true, request, message: 'Заявка создана, средства списаны' });
    } catch (e) {
        ...
        if (action === 'approve') {
            ...
            if (request.type === 'withdraw') {
                await User.findByIdAndUpdate(user._id, {
                    ...
                    $push: {
                        transactions: {
                            $each: [{ name: `Withdraw rejected: refund ${request.amount} MMO`, amount: request.amount, time: new Date() }],
                            $position: 0,
                            $slice: 30
                        }
                    }
...
app.get('/api/admin/metrics', adminAuthMiddleware, async (req, res) => {
    try {
        ...
        const errorRate5xx = requestsLastMinute > 0 ? (errorsLastMinute / requestsLastMinute * 100).toFixed(3) : 0;
...
mongoose.connection.once('open', async () => {
    ...
    console.log('⚔️ PvP Арена: активна (WebSocket)');
    console.log('📢 Рассылки: активны');
});
