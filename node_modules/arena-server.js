// ============================================
// ARENA PVP SERVER (Socket.IO) - UPDATED
// ============================================
...
const LEAGUES = {
    'BRONZE': { min: 0, max: 999, color: '#cd7c3a', reward: 100, icon: '🥉' },
    'SILVER': { min: 1000, max: 1999, color: '#94a3b8', reward: 150, icon: '🥈' },
    'GOLD': { min: 2000, max: 2999, color: '#f59e0b', reward: 250, icon: '🥇' },
    'PLATINUM': { min: 3000, max: 3999, color: '#06b6d4', reward: 400, icon: '💎' },
    'DIAMOND': { min: 4000, max: 9999, color: '#a855f7', reward: 600, icon: '👑' }
};
...
function getLeagueProgress(rating) {
    ...
    for (let i = 0; i < leagues.length; i++) {
        const [league, data] = leagues[i];
        if (rating >= data.min && rating <= data.max) {
            const nextLeague = leagues[i + 1];
            if (nextLeague) {
                const needed = nextLeague[1].min - rating;
                return { currentLeague: league, nextLeague: nextLeague[0], needed };
            }
            return { currentLeague: league, nextLeague: null, needed: 0 };
        }
    }
    return { currentLeague: 'BRONZE', nextLeague: 'SILVER', needed: 1000 - rating };
}
...
function createBattleMonster(creature, multiplier = 1) {
    const baseIncome = creature.incomeBase || 10;
    return {
        id: creature.id,
        name: creature.name,
        icon: creature.icon,
        rarity: creature.rarity,
        atk: Math.floor(baseIncome * 2 * multiplier),
        def: Math.floor(baseIncome * 1.5 * multiplier),
        hp: Math.floor(baseIncome * 10 * multiplier),
        maxHp: Math.floor(baseIncome * 10 * multiplier)
    };
}
...
// Process battle move
function processMove(battle, attackerId, attackerIndex, targetIndex) {
...
function formatBattleStart(battle, perspective) {
    ...
    return {
        ...
        player: {
            ...
            monsters: isPlayer ? battle.player.monsters : battle.opponent.monsters,
        ...
        opponent: {
            ...
            monsters: isPlayer ? battle.opponent.monsters : battle.player.monsters,
...
function setupArenaServer(io) {
    io.on('connection', (socket) => {
        ...
        socket.on('find-match', async (data) => {
            ...
            const uniqueIds = new Set(data.team.map(m => m.id));
            if (uniqueIds.size !== data.team.length) {
                socket.emit('error', { message: 'Cannot select duplicate monsters!' });
                return;
            }
        ...
        socket.on('accept-battle', async (data) => {
            ...
            if (battle.player.accepted && battle.opponent.accepted) {
                ...
                battle.player.totalHealth = battle.player.monsters.reduce((sum, m) => sum + m.hp, 0);
                battle.player.maxHealth = battle.player.monsters.reduce((sum, m) => sum + m.maxHp, 0);
                battle.opponent.totalHealth = battle.opponent.monsters.reduce((sum, m) => sum + m.hp, 0);
                battle.opponent.maxHealth = battle.opponent.monsters.reduce((sum, m) => sum + m.maxHp, 0);
